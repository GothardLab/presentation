...
int fixSpotFailMax = 3; # this is the number of times the monkey can fail the fix spot
int fixSpotAttempts = 0;
...
int itiDuration=2000; #duration of the inter-trial-interval (ms)
...
string resultfile="results_";
resultfile.append(date_time("dd"));
resultfile.append(date_time("_"));
resultfile.append(date_time("hh"));
resultfile.append(date_time("_"));
resultfile.append(date_time("nn"));
resultfile.append(".txt");
term.print(resultfile);
ofile1.open( resultfile, false );
...
loop until fixdone>numberofSpots begin;

#fixSpotAttempts = fixSpotAttempts+1;

fixspotX=fixpos[fixlist[1]][1];
fixspotY=fixpos[fixlist[1]][2];
fixspot_copy.set_part_x( 3, fixpos[fixlist[1]][1]); #set the X position of the fixspot on the experimenter's monitor
fixspot_copy.set_part_y( 3, fixpos[fixlist[1]][2]); #set the Y position of the fixspot on the experimenter's monitor
fixspot_copy.set_part_x( 2, fixpos[fixlist[1]][1]); #set the X position of the black box on the experimenter's monitor
fixspot_copy.set_part_y( 2, fixpos[fixlist[1]][2]); #set the Y position of the black box on the experimenter's monitor
fixspot_copy.set_part_x( 1, fixpos[fixlist[1]][1]); #set the X position of the red box on the experimenter's monitor
fixspot_copy.set_part_y( 1, fixpos[fixlist[1]][2]); #set the Y position of the red box on the experimenter's monitor
fixspot_real.set_part_x( 1, fixpos[fixlist[1]][1]); #set the X position of the fixspot on the monkey's monitor
fixspot_real.set_part_y( 1, fixpos[fixlist[1]][2]); #set the Y position of the fixspot on the monkey's monitor
...
encode(1000+int(double(fixpos[fixlist[1]][1])));
encode(1000+int(double(fixpos[fixlist[1]][2])));
...
#~~~~~~~~~~~~~~~~ IF THE MONKEY MAINTAINED HIS FIXATION FOR THE REQUIRED AMOUNT OF TIME THEN GIVE JUICE AND BEGIN THE ITI ~~~~~~~~
	...
	if  fixSpotAttempts > fixSpotFailMax 
	then
		fixdone=fixdone+1;
	end;
