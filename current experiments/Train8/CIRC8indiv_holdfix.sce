...
picture{
	...
	#draw a white box, this is the fixspot, height/width are temporary and are updated later
	box {color = 0,255,0;height=5;width=5;display_index=1;}fixspot2;
	x=0; y=0; #the position of the fixspot, this can be edited in the PCL code
...
picture{
	...
	on_top=true; 
		...
		box {color = 255,255,255;height=10;width=10;display_index=2;}fixspotBoundary2;
	x=0; y=0;  #the position of the fixspot boundary, this can be edited in the PCL code
on_top=true; #draw the fixspot on top layer of the picture
...
picture{
	...
	box {color = 0,255,0;height=5;width=5;display_index=1;}fiximagebound;
	x=0; y=0; #the position of the fixspot, this can be edited in the PCL code
...
int fixwin=22; #this is the extra room that the monkey has around the edge of the fixspot (pixels)
...
int fixationDuration=25; # time required for the monkey to maintain fixation on the fixspot (ms)
int imageDuration=2700; # time required for the monkey to look at the image (ms)
int imagewin=24; #this is the extra room that the monkey has around the edge of the image (pixels)
...
int itiDuration=7000; #duration of the inter-trial-interval (ms)
int numberOfJuiceDrops=20;	# the duration of the TTL pulse that will trigger the juice pump
...
fixspotBoundary2.set_height(5+2*imagewin); # set the height boundary of the fixspot (the extra space the monkey is allowed)
fixspotBoundary2.set_width(5+2*imagewin); # set the width boundary of the fixspot (the extra space the monkey is allowed)
		...
		encode(05); #encode fail to hold
			...
			#~~~~~~~~~~~~~~~~ IF THE MONKEY MAINTAINED HIS FIXATION FOR THE REQUIRED AMOUNT OF TIME THEN GIVE JUICE AND BEGIN THE ITI ~~~~~~~~
				...
				loop until iscan_x>double(int(w/2.0)+imagewin) || iscan_x<double(-int(w/2.0)-imagewin) ||
					...
					(clock.time()-clockticker)>(imageDuration+random(1,150))
				begin
					iscan_x =iscan.read_analog(idX)*ratioX; # get the X eye position
					iscan_y =iscan.read_analog(idY)*ratioY; # get the Y eye position
					image_copy.set_part_x( 5, iscan_x ); # set the X eye position to eyeposition of the picture
					image_copy.set_part_y( 5, iscan_y ); # set the Y eye position to eyeposition of the picture
