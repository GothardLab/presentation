...
int fixspotsize=6;  # size of fixspot (pixels)
int fixwin=16; #this is the extra room that the monkey has around the edge of the fixspot (pixels)
...
int itiDuration=1500; #duration of the inter-trial-interval (ms)
int numberOfJuiceDrops=1;	# the duration of the juice TTL pulse
int clockticker=0; # just an integer counter to hold the clock time
int numberofSpots=2; # just an integer counter to hold the clock time
...
int fixdone=1;
int numspots=numberofSpots+random(0,1);
...
itiDur=random(500,750);		
	...
	#	int indexthing=0;
		cuespot.present();	#present the fixspot
	...
	iti_correct.set_duration(random(350,500));
		...
		int movie_encode=cnds_to_show2[current_cnd];
			...
			oportPulse.send_code(128,1);
			wait_interval(5);
			pic1.set_part( 1, frame_bmp[startframe[cnds_to_show2[current_cnd]]] ); 
		loop frame_val=startframe[cnds_to_show2[current_cnd]] until frame_val==endframe[cnds_to_show2[current_cnd]] begin;
