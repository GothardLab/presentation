...
int fixspotsize=13;  # size of fixspot (pixels)
int fixwin=26; #this is the extra room that the monkey has around the edge of the fixspot (pixels)
...
int imageDuration=2000; # time required for the monkey to look at the image (ms)
int imagewin=50; #this is the extra room that the monkey has around the edge of the image (pixels)
					# so, if image win is 40, then there is a leniency of 10 pixels on all sides of the fixspot
int imagewidth=720; #the width that all images should be standardized to (pixels)
int imageheight=480; #the height that all images should be standardized to (pixels)
...
string exp_par="Zin_ShowImages_experimental_parameters.txt"; #the name of the parameter file that contains information about itm files, cnd files
