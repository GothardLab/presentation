...
picture{
	...
	#draw a black box, this is still part of the image boundary, height/width are temporary and are updated later
	box {color = 0,0,0; height=9;width=9; display_index=2;}imageBoundaryBlackA;
	x=0; y=0; #the position of the image boundary, this is updated according to the condition
	on_top=true; #draw the black box on top of the red box: this makes a red boundary
...
#~~~~~~~~~~~~~~~~~~~~~ SET THE IMAGE SIZE ~~~~~~~~~~~~~~~~~~
placeholderA1.set_load_size(double(imageheight),double(imagewidth),0.0); #set the width and height of the averted gaze image on the monkey's monitor
placeholderA2.set_load_size(double(imageheight),double(imagewidth),0.0); #set the width and height of the averted gaze image on the experimenter's monitor
placeholderD1.set_load_size(double(imageheight),double(imagewidth),0.0); #set the width and height of the direct gaze image on the monkey's monitor
placeholderD2.set_load_size(double(imageheight),double(imagewidth),0.0); #set the width and height of the direct gaze image on the experimenter's monitor
...
loop current_trial = 1 until current_trial > num_trials begin 
		...
		loop current_cnd=1 until current_cnd>cnds_to_show.count() begin	#loop so long as there is 1 condition to show		
			...
			#if there are multiple bout durations for the directed gaze image then the below loop can
			#be used to set different duration for different individuals. If only one duration is used
			#then comment out the loop and if/elseif statements and just leave "int boutDuration = 0;"
			#and "boutDuration = boutDurs[1];" lines
		...
		#SET X AND Y OFFSETS	
		#Shifts center of these various parts of the picture to the XY coordinates in the itm file
				image_averted2.set_part_x( 3, X_offset); #set the X position of the image on the experimenter's monitor
				image_averted2.set_part_y( 3, Y_offset); #set the Y position of the image on the experimenter's monitor
				image_averted2.set_part_x( 2, X_offset); #set the X position of the black box on the experimenter's monitor
				image_averted2.set_part_y( 2, Y_offset); #set the Y position of the black box on the experimenter's monitor
				image_averted2.set_part_x( 1, X_offset); #set the X position of the red box around the image on the experimenter's monitor
				image_averted2.set_part_y( 1, Y_offset); #set the X position of the red box around the image on the experimenter's monitor
				image_averted1.set_part_x( 1, X_offset); #set the X position of the image on the monkey's monitor
				image_averted1.set_part_y( 1, Y_offset); #set the X position of the image on the monkey's monitor
				
				image_direct2.set_part_x( 3, X_offset); #set the X position of the image on the experimenter's monitor
				image_direct2.set_part_y( 3, Y_offset); #set the Y position of the image on the experimenter's monitor
				image_direct2.set_part_x( 2, X_offset); #set the X position of the black box on the experimenter's monitor
				image_direct2.set_part_y( 2, Y_offset); #set the X position of the black box on the experimenter's monitor
				image_direct2.set_part_x( 1, X_offset); #set the X position of the red box around the image on the experimenter's monitor
				image_direct2.set_part_y( 1, Y_offset); #set the X position of the red box around the image on the experimenter's monitor
				image_direct1.set_part_x( 1, X_offset); #set the X position of the image on the monkey's monitor
				image_direct1.set_part_y( 1, Y_offset); #set the X position of the image on the monkey's monitor
				...
				#~~~~~~~~ WAIT FOR THE MONKEY TO LOOK AT THE FIXSPOT ~~~~~~~~~~~~~~~~
				iscan_x = iscan.read_analog(idX)*ratioX; # get the X eye position
				iscan_y = iscan.read_analog(idY)*ratioY; # get the Y eye position
				clockticker = clock.time(); #get the time
