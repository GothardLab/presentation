...
int trialDuration = 8000; # duration of trial (ms)
...
int imagewidth = 585; #the width that all images should be standardized to (pixels)
...
int x_off_index = current_filename.find("x");
int y_off_index = current_filename.find("y");
...
loop i = 1 until i>(num_items) begin  #loop through each line in the item file
   current_filename = fil.get_line();	#get the list of characters that compose the current line in the itm file
	...
	term.print("\nreading ");
	term.print(y_off_index);
	term.print("-->");
	term.print(y_off_index+3);
	
	string debug = current_filename.substring(y_off_index,3);
...
#  current_filename=fil.get_line();	#get the list of characters that compose the current line in the itm file
#	x_offsets[i] = int(double(current_filename.substring(x_off_index,3)));
#	y_offsets[i] = int(double(current_filename.substring(y_off_index,3))); #get just the item filename from that line
