...
#		v0.5	02/19/16		Moved monitors, added juice, added hold-limit, added 
...
int numberOfJuiceDrops = 3;		# Number of juice drops to give monkey for a correct response
int holdTimeLimit = 2000;			# Time (ms) which the monkey can hold the screen before getting an error
int bottomTouchLimit = -400;		# Pixel space below which the monkey can touch free (so that they can hold bottom of monitor)
...
begin
	...
	begin
		juicePulse.send_code(1,10);	# Give juice to the monkey
		wait_interval(110);
		nojo=nojo+1;
	end;
end;
