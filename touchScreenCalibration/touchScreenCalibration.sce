...
#This program is designed to calibrate the ISCAN eyetracking system on a tilted screen
...
picture{
	...
	box {color=0,255,0;height=7;width=7;display_index=2;};
...
double monitorX=1080; 								# Monitor width (pixels)1024.0
double monitorY=1024; 								# Monitor height (pixels)768.0
...
int fixspotsize=15; 									# Size of fixspot (pixels), 7px @800x600 / 10@1080
int fixwin=35; #30 #this is the extra room that the monkey has around the edge of the fixspot (pixels), usually 20 @800x600q
...
int fixationDuration=150;#120 # time required for the monkey to maintain fixation on the fixspot (ms)
...
output_port oportJuice=output_port_manager.get_port(1); # open the port that will send the juice TTL pulse
...
#~~~~~~~~~~~~~~~~ IF THE MONKEY MAINTAINED HIS FIXATION FOR THE REQUIRED AMOUNT OF TIME THEN GIVE JUICE AND BEGIN THE ITI ~~~~~~~~
		...
		oportJuice.send_code(10,1150);#give juice to the monkey
		wait_interval(150);
		oportJuice.send_code(0,150);	# Give juice to the monkey
		wait_interval(150);
