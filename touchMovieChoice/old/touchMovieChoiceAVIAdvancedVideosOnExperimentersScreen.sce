   ...
   stimulus_event {
	 ...
	 stimulus_event {
		picture {
			...
			x = 450; y = -150;# String to display touch position
			on_top=true; 
		}stimulusPictureExperimenter;
	} stimulusEventExperimenter;	

	video {
		display_index=2;
		filename = "test1.avi";
		x = -300; y = 200;
	}leftVideoExperimenter;
				
	video {
		display_index=2;
		filename = "test2.avi";
		x = 300; y = 200;
	}rightVideoExperimenter;
	...
	begin	
		...
		if (response_manager.response_data_count() > response_count) then
			...
			then	
				...
				stimulusPictureExperimenter.set_part_x( 5, monkeyX );  
				stimulusPictureExperimenter.set_part_y( 5, monkeyY );
...
begin
   ...
   string monkeyResponseStr = "Ignore"; 	# String to return with results of trial, defaults to ignored
	...
	stimulusPictureExperimenter.set_part_x( 2, leftStimulusX); 				# Set the X position of the black touch box on the experimenter's monitor
	stimulusPictureExperimenter.set_part_y( 2, leftStimulusY); 				# Set the Y position of the black touch box on the experimenter's monitor
	stimulusPictureExperimenter.set_part_x( 1, leftStimulusX); 				# Set the X position of the red outline on the experimenter's monitor
	stimulusPictureExperimenter.set_part_y( 1, leftStimulusY); 				# Set the Y position of the red outline on the experimenter's monitor
	...
	stimulusPictureExperimenter.set_part_x( 4, rightStimulusX); 				# Set the X position of the black touch box on the experimenter's monitor
	stimulusPictureExperimenter.set_part_y( 4, rightStimulusY); 				# Set the Y position of the black touch box on the experimenter's monitor
	stimulusPictureExperimenter.set_part_x( 3, rightStimulusX); 				# Set the X position of the red outline on the experimenter's monitor
	stimulusPictureExperimenter.set_part_y( 3, rightStimulusY); 				# Set the Y position of the red outline on the experimenter's monitor
	
	stimulusPictureExperimenter.set_part_x( 6, -9999);	# Move the left touchCue off the screen on the experimenter's monitor
	stimulusPictureExperimenter.set_part_y( 6, 9999); 	# Move the left touchCue off the screen on the experimenter's monitor
	stimulusPictureExperimenter.set_part_x( 7, 9999); 	# Move the right touchCue off the screen on the experimenter's monitor
	stimulusPictureExperimenter.set_part_y( 7, 9999);	# Move the right touchCue off the screen on the experimenter's monitor


	stimulusPictureExperimenter.present();
	...
	until false begin
		if (clock.time() >= next_frame) then
			...
			then
				...
				stimulusPictureMonkey.set_part_y(1, (leftStimulusY+touchCueYOffset) );
				stimulusPictureMonkey.set_part_x(2, rightStimulusX);
				stimulusPictureMonkey.set_part_y(2, (rightStimulusY+touchCueYOffset) );
				
				stimulusPictureExperimenter.set_part_x( 6, leftStimulusX); 						# Set the X position of the left touch cue
				stimulusPictureExperimenter.set_part_y( 6, leftStimulusY +touchCueYOffset); # Set the Y position of the red outline on the experimenter's monitor
				stimulusPictureExperimenter.set_part_x( 7, rightStimulusX); 						# Set the X position of the left touch cue
				stimulusPictureExperimenter.set_part_y( 7, rightStimulusY +touchCueYOffset);# Set the Y position of the red outline on the experimenter's monitor
				stimulusPictureExperimenter.present();
...
begin 
	...
	begin
		...
		# Pre-load the left movie
		leftVideoExperimenter.set_filename(leftMovieName+".avi");
		leftVideoExperimenter.set_logging(true);
		leftVideoExperimenter.prepare();
		
		# Pre-load the right movie
		rightVideoExperimenter.set_filename(rightMovieName+".avi");
		rightVideoExperimenter.set_logging(true);
		rightVideoExperimenter.prepare();
