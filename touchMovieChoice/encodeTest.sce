...
#
#	Verison History
#		v0.1	03/01/16		Branched touchImageChoice from touchTrain
...
int estimatedTrials  =	0;			# Number of total touchbox presentations (numberTrails * numberConditions)
   ...
   string monkeyResponseStr = "Ignore"; 	# String to return with results of trial, defaults to ignored
	...
	until false begin
		if (clock.time() >= next_frame) then
			...
			then
				# Move the touch cues into position on the monkey's screen
				...
				# Move the touch cues into position on the experimenter's screen
			...
			elseif # If the monkey has made a touch response
				(response_manager.response_data_count() > response_count)
			then
						...
						then
							...
							then
								...
								then
									...
									presentIncorrectFeedback();
									term.print_line("Monkey touched early");
								elseif
									monkeyX <= rightUpperX && monkeyX >= rightLowerX && currentY <= rightUpperY && currentY >= rightLowerY
								then
									term.print_line("D\n");
...
term.print_line( " Correct touches: " + string( trialsCompleted ) + "/" + string(trialsAttempted));
term.print_line( "  Missed touches: " + string( missedTrials ) + "/" + string(trialsAttempted) );
term.print_line( "Start Cue errors: " + string( startCueMisses ) + "/" + string(trialsAttempted) );
term.print_line( " Ignored touches: " + string( ignoredTrials ) + "/" + string(trialsAttempted) );
