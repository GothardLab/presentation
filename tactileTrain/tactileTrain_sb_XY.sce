...
picture{
	...
	box {color = 200,200,200;height=15;width=15;display_index=1;}touchfeedback;
	x=0; y=0;
	on_top=true;
...
int startCueMissItiDuration		=	2000; # Duration of the inter-trial-interval(ms) for early touches
int missedItiDuration	=	2000; # Duration of the inter-trial-interval(ms) for missed trials
int ignoredItiDuration   =	2000;	# Duration of the inter-trial-interval(ms) for ignored trials
...
int bottomTouchLimit = -425;		# Pixel space below which the monkey can touch free (so that they can hold bottom of monitor)
...
int rewardDelayTime = 2000;		#Time (ms) which the monkey has to wait to receive reward
			...
			itiMonkey.present();
				...
				then
					...
					then
						...
						#wait_interval(500); # delay between bar and targets
...
begin
	...
	if
		choice == "Left"
	then
		stimulusMonkey.set_part_x( 1, 300); 			#Move the left stimulus on feedback position on the monkey's screen
		...
		stimulusExperimenter.set_part_x( 3, 300); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_y( 3, 450); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_x( 2, 300); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_y( 2, 450); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_x( 1, 300); 	#Move the right stimulus off the the experimenter's monitor
	...
	then
		...
		stimulusMonkey.set_part_x( 2, 300); 			#Move the right stimulus on feedback position on the monkey's screen
	   ...
	   stimulusMonkey.set_part_x( 3, 9999); 			#Move touchfeedback off the monkey's screen
		...
		stimulusExperimenter.set_part_x( 6, 300); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_y( 6, 450); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_x( 5, 300); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_y( 5, 450); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_x( 4, 300); 	#Move the right stimulus off the the experimenter's monitor
...
begin
   ...
   string monkeyResponseSub = ""; # String to return with results of trial
	...
	#stimulusYPosition=random(-275,175);# RANDOMIZE Y POSITION OF STIMULUS
...
#term.print(randx); term.print(randx2); term.print(randy); term.print(randy2);
	...
	double leftLowerY = leftStimulusY - lowerY;
	...
	double rightLowerY = rightStimulusY - lowerY;
						...
						#help=0; # reset help counter
							...
							then # Record that the monkey has made a correct touch
								...
								#giveJuiceReward ( leftJuiceValue ) #juiceDrops
								 stimulusMonkey.set_part_x( 3, currentX); 			#Move touchfeedback on the monkey's screen
		                   stimulusMonkey.set_part_y( 3, currentY); 			#Move touchfeedback on the monkey's  screen
						       stimulusMonkey.present();
								wait_interval(500);
		                   ...
		                   stimulusMonkey.set_part_y( 3, currentY); 			#Move touchfeedback on the monkey's  screen
							...
							elseif help<=limhelp then # give another chance to the monkey (need encode...)
							help=help+1;
						   stimulusMonkey.set_part_x( 3, currentX); 			#Move touchfeedback off the monkey's screen
		               stimulusMonkey.set_part_y( 3, currentY); 			#Move touchfeedback off the monkey's  screen
							stimulusMonkey.present();	
							stimulusExperimenter.present();
							hasTouched = 0;
