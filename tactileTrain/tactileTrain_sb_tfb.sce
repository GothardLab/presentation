...
#	which is the pixel value of the center of the monkey's touchscreen.hastouched
#
#	Verison History
...
picture{
	...
	bitmap {display_index=1;filename="placeholder.bmp";
		width=325;
		height=325;}leftImageMonkey;
		x=-300; y=0;	# Position of the image on screen
		
	bitmap {display_index=1;filename="placeholder.bmp";
		width=325;
		height=325;}rightImageMonkey;
		x=300; y=0;	# Position of the image on screen
		
	box {color = 200,0,0;height=30;width=30;display_index=1;}touchfeedback;
	x=0; y=0;
...
picture{
	...
	bitmap {display_index=2;filename="placeholder.bmp";
		width=325;
		height=325;}leftImageExperimenter;
		x=-300; y=0;	# Position of the image on screen
		on_top=true;
	...
	bitmap {display_index=2;filename="placeholder.bmp";
		width=325;
		height=325;}rightImageExperimenter;
		x=300; y=0;	# Position of the image on screen
		on_top=true;
...
int touchBoxSize	=	340;			# size of touch box (pixels)
int touchWindow	=	40; 			# this is the extra room that the monkey has around the edge of the touch box (pixels)
...
int startCueSize = 150;				# size of start cue box (pixels)
int startCueWindow =100;				# Size of window, in pixels, around the start cue
int stimulusTime	=	5000; 		# time that monkey is allowed to get to touch box (ms) per trial
int correctItiDuration	=	1500; # Duration of the inter-trial-interval(ms)for correct trials
int startCueMissItiDuration		=	1500; # Duration of the inter-trial-interval(ms) for early touches
int missedItiDuration	=	1500; # Duration of the inter-trial-interval(ms) for missed trials
int ignoredItiDuration   =	1500;	# Duration of the inter-trial-interval(ms) for ignored trials
...
int bottomTouchLimit = -375;		# Pixel space below which the monkey can touch free (so that they can hold bottom of monitor)
...
int leftStimulusYPosition = startCueY;# Y position of the left stimulus
int rightStimulusYPosition = startCueY;# Y position of the right stimulus
int stimulusXPosition = startCueX;			# X position of both stimuli
int spaceY=215;							# Value added to place the targets away from the cue
int spaceX=275;							# Value added to place the targets away from the cue
int rewardValueTextYPosition = -300;	# Y position of text with reward value
int rewardDelayTime = 4000;		#Time (ms) which the monkey has to wait to receive reward
bool pauseTaskForGrooming = false; # Boolean variable to pause the task after stimulus choice
int groomingTime =6000;  			#Time (ms) which the experimenter has to groom/touch the monkey 
bool showPreTrialCue = true;		#Boolean, set to true to show the monkey a pre-trial touch cue to initate the trial
int RAND=0; # RANDOM TARGET POSITION 0=NO, 1=YES
...
int right_index=cnd_name.find("rght");

int left_posX=cnd_name.find("LposX");
int right_posX=cnd_name.find("RposX");
int left_posY=cnd_name.find("LposY");
int right_posY=cnd_name.find("RposY");
...
array<string>left_itm_posx[num_cnds];
array<string>right_itm_posx[num_cnds]; 
array<string>left_itm_posy[num_cnds];
array<string>right_itm_posy[num_cnds]; 				# Create an array which will contain the item position for each condition  			
...
begin							# Loop through each line in the cnd file
	...
	right_itm[i]=cnd_name.substring(right_index,3);	# Get just the item numbers (not the condition number) for the right00 side	
	
	term.print_line("left_index" + string(left_index) + "right_index" + string(right_index) + "left_posx" + string(left_posX)+"left_posy" + string(left_posY) + "right_posx" + string(right_posX) + "right_posy" + string(right_posY));
	
	
	left_itm_posx[i]=cnd_name.substring(left_posX,4);
	left_itm_posy[i]=cnd_name.substring(left_posY,4);
	right_itm_posx[i]=cnd_name.substring(right_posX,4);
	right_itm_posy[i]=cnd_name.substring(right_posY,4);
	
	term.print_line("left_itm_index" + left_itm[i] + "right_itm_index" + right_itm[i] + "left_itm_posx" + left_itm_posx[i] + "left_itm_posy" + left_itm_posy[i] +"right_itm_posx" + right_itm_posx[i]+ "right_itm_posy" + right_itm_posy[i]);
			...
			then	
				...
				then			
					...
					then # Record that the monkey has made a correct touch
						...
						wait_interval(250) # GIVE TIME FOR THE MONKEY AFTER CUE
			...
			itiMonkey.present();
				...
				then
					...
					then
						...
						#wait_interval(2000); # delay between bar and targets
						
						postStimClockticker=clock.time();
						loop until # Loop until time is up
							(clock.time()-postStimClockticker) > 750
						begin
...
begin
	...
	if leftItemEmpty == false then
		leftJuiceValue = juiceDrops[int(double(left_itm[ordered_cnds_to_show[currentConditionIdx]].substring(1,3)))];
	else
		leftJuiceValue = 0;			
	end;
	
	if rightItemEmpty == false then
		rightJuiceValue = juiceDrops[int(double(right_itm[ordered_cnds_to_show[currentConditionIdx]].substring(1,3)))];			
	else
		rightJuiceValue = 0;			
	end;
	...
	if
		choice == "Left"
	then
		stimulusMonkey.set_part_x( 1, startCueX); 			#Move the left stimulus on feedback position on the monkey's screen
		stimulusMonkey.set_part_y( 1, 450); 			#Move the left stimulus on feedback position on the monkey's screen
		...
		stimulusMonkey.set_part_x( 3, 9999); 			#Move touchfeedback off the monkey's screen
		stimulusMonkey.set_part_y( 3, 9999); 			#Move touchfeedback off the monkey's  screen
		stimulusMonkey.present();							#Update the monkey's screen
		
		
		stimulusExperimenter.set_part_x( 3, startCueX); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_y( 3, 450); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_x( 2, startCueX); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_y( 2, 450); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_x( 1, startCueX); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_y( 1, 450); 	#Move the right stimulus off the the experimenter's monitor
	...
	then
		...
		stimulusMonkey.set_part_x( 2, startCueX); 			#Move the right stimulus on feedback position on the monkey's screen
		stimulusMonkey.set_part_y( 2, 450); 			#Move the right stimulus on feedback position on the monkey's screen
	   ...
	   stimulusMonkey.set_part_x( 3, 9999); 			#Move touchfeedback off the monkey's screen
		...
		stimulusExperimenter.set_part_x( 6, startCueX); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_y( 6, 450); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_x( 5, startCueX); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_y( 5, 450); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_x( 4, startCueX); 	#Move the right stimulus off the the experimenter's monitor
		stimulusExperimenter.set_part_y( 4, 450); 	#Move the right stimulus off the the experimenter's monitor
...
begin
   ...
   string monkeyResponseSub = ""; # String to return with results of trial
	...
	double leftJuiceValue =  0;
	double rightJuiceValue = 0; 
	double leftStimulusX = 0;
	double leftStimulusY = 0;
	double rightStimulusX = 0;
	double rightStimulusY = 0;
	
  

	double randx=0; double randy=0; double randx2=0; double randy2=0;
	if RAND==1 then #RANDOMIZE Y POSITION OF STIMULUS
	loop until randx!=0 && randy!=0 && randx2!=0 && randy2!=0 && (randx!=randx2 || randy!=randy2) && (randx+randy)<2 #&& (randx2+randy2)<2 # COMMENT TWO LAST CONDITION FOR ONLY 3 POSITIONS
	begin
	randx=round(random(-14,14)/10,0);
	randy=round(random(-14,14)/10,0);
	randx2=round(random(-14,14)/10,0);
	randy2=round(random(-14,14)/10,0);
   end;
end;

#term.print(randx); term.print(randx2); term.print(randy); term.print(randy2);
if RAND==1 then
	wait_interval(50);
	if randx == 1 && randy == 1 then
		encode(32); #top right hand corner, for "left" target
	elseif randx == 1 && randy == -1 then 
		encode(33); #bottom right hand corner, for "left" target
	elseif randx == -1 && randy == 1 then 
		encode(31); #top left hand corner, for "left" target
	elseif randx == -1 && randy == -1 then 
		encode(34); #bottom left hand corner, for "left" target
	end;
	
	wait_interval(50);
	if randx2 == 1 && randy2 == 1 then
		encode(42); #top right hand corner, for "right" target
	elseif randx2 == 1 && randy2 == -1 then 
		encode(43); #bottom right hand corner, for "right" target
	elseif randx2 == -1 && randy2 == 1 then 
		encode(41); #top left hand corner, for "right" target
	elseif randx2 == -1 && randy2 == -1 then 
		encode(44); #bottom left hand corner, for "right" target
	end;
end;
	
	if leftItemEmpty == false then
		if RAND==1 then
		leftStimulusY = leftStimulusYPosition+(spaceY*randy);			
		leftStimulusX = stimulusXPosition+(spaceX*randx);
    else
	
	leftStimulusY=int(double(left_itm_posy[ordered_cnds_to_show[currentConditionIdx]]));
	leftStimulusX=int(double(left_itm_posx[ordered_cnds_to_show[currentConditionIdx]]));
	...
	if rightItemEmpty == false then
		if RAND==1 then
		rightStimulusY = rightStimulusYPosition+(spaceY*randy2);			
		rightStimulusX = stimulusXPosition+(spaceX*randx2);
else	
	rightStimulusY=int(double(right_itm_posy[ordered_cnds_to_show[currentConditionIdx]]));
	rightStimulusX=int(double(right_itm_posx[ordered_cnds_to_show[currentConditionIdx]]));
...
#rightStimulusX=-150;
   ...
   stimulusMonkey.set_part_x( 3, 9999); 			#set the X position of the touchfeedback box on the monkey's monitor
	stimulusMonkey.set_part_y( 3, 9999); 			#set the Y position of the touchfeedback box on the monkey's monitor
	...
	double leftUpperY = leftStimulusY + upperY;
	double leftLowerY = leftStimulusY - upperY; # lowerY before
	double leftUpperX = leftStimulusX + upperX;
	double leftLowerX = leftStimulusX - upperX;
	
	# Offset existing pixel boundry for correct touch on right stimulus	
	double rightUpperY = rightStimulusY + upperY;
	double rightLowerY = rightStimulusY - upperY; # lowerY before
	double rightUpperX = rightStimulusX + upperX;
	double rightLowerX = rightStimulusX - upperX;
	...
	hasTouched = 0; 
   help=0;# rest help counter 
	
	loop until # Loop until time is up or the monkey has touched
			(clock.time()-clockticker) > stimulusTime || hasTouched == 1 #|| help>limhelp
	begin
						...
						#help=0; # reset help counter
							...
							then # Record that the monkey has made a correct touch
								...
								#giveJuiceReward ( leftJuiceValue ) #juiceDrops
								 stimulusMonkey.set_part_x( 3, monkeyX); 			#Move touchfeedback on the monkey's screen
		                   stimulusMonkey.set_part_y( 3, monkeyY); 			#Move touchfeedback on the monkey's  screen
						       stimulusMonkey.present();
								wait_interval(500);
		                   ...
		                   stimulusMonkey.set_part_y( 3, monkeyY); 			#Move touchfeedback on the monkey's  screen
							...
							elseif help<=limhelp && hasTouched==0 && state == "Down" then # give another chance to the monkey (need encode...)
							help=help+1;
						   stimulusMonkey.set_part_x( 3, monkeyX); 			#Move touchfeedback off the monkey's screen
		               stimulusMonkey.set_part_y( 3, monkeyY); 			#Move touchfeedback off the monkey's  screen
							stimulusMonkey.present();	
							stimulusExperimenter.present();
							hasTouched = 0;
							encode(monkeyResponseCode);
...
begin 
	...
	begin
		...
		encode(trialStartCode); # Send the trial start encode
		
		#Get the current condition value
		currentConditionVal = ordered_cnds_to_show[currentConditionIdx];
		
		term.print_line("ConditionValue" + string(currentConditionVal));
		
		wait_interval(100);
		encode(currentConditionVal + 100); # Encode the current condition + 100 as an unique offset
		
		
		#Find which items are in this condition
		leftItemNum = int(double(left_itm[currentConditionVal].substring(1,3)));
		rightItemNum = int(double(right_itm[currentConditionVal].substring(1,3)));
		leftItemEmpty = false;
		rightItemEmpty = false;
		
		wait_interval(100);
		encode(leftItemNum + 150); # Encode the current left item number + 150 as an unique offset
		wait_interval(100);
		encode(rightItemNum + 150); # Encode the current left item number + 150 as an unique offset		
		
		#If the left item value isn't empty (value of zero), load it
		if leftItemNum > 0 then
			string leftImageFilename = images[int(double(left_itm[currentConditionVal].substring(1,3)))]+".bmp";
			...
			leftItemEmpty = true;
			term.print(leftItemEmpty)
		end;
		
		#If the right item value isn't empty (value of zero), load it
		if rightItemNum > 0 then
			string rightImageFilename = images[int(double(right_itm[currentConditionVal].substring(1,3)))]+".bmp";
...
wait_interval(50);		
		...
		encode(trialEndCode); # Send the trial start encode
		wait_interval(50);
