...
picture{
	...
	x=0; y=250;  #the position of the fixspot boundary, this can be edited in the PCL code
	...
	x=0; y=250; #the position of the fixspot, this can be edited in the PCL code
...
int trialDuration = 5000; # duration of trial (ms)
				...
				begin
					...
					if timeInEye >= eyeDuration then # time spent fixating eye region exceeds threshold for triggering direct gaze
						...
						#/\/\/\/\/\/\/\/\ RETURN TO AVERTED GAZE /\/\/\/\/\/\/\/\
							...
							refractoryEnd = refractoryStart + 6000; # set time at which averted gaze will become eligible for direct gaze bout
